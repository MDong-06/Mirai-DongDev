  ...
  setInterval(async() => {
    ...
    if (day != day_now) {
      ...
      checkttData.forEach(async(checkttFile) => {
        ...
        let checkttBody = '💀==𝐓𝐨𝐩 10 𝐓ươ𝐧𝐠 𝐓á𝐜 𝐍𝐠à𝐲==💀\n';
      ...
      if (day_now == 1) {
        ...
        checkttData.forEach(async(checkttFile) => {
          ...
          let checkttBody = '🎃==𝐓𝐨𝐩 10 𝐓ươ𝐧𝐠 𝐓á𝐜 𝐓𝐮ầ𝐧==🎃\n';
          checkttBody += storage.slice(0, 10).map(item => {
            return `${count++}. ${item.name} (${item.count})`;
        }).join('\n');
          api.sendMessage(checkttBody, checkttFile.replace('.json', ''), (err) => err ? console.log(err) : '');
...
(async function () {
	...
	logger(`${api.getCurrentUserID()}  ▷ ${global.config.PREFIX} ◁ • ${(!global.config.BOTNAME) ? "This bot was made by CatalizCS and SpermLord\nBot Halloween Edit By Nguyen 🎃" : global.config.BOTNAME}`, "[ BOT INFO ]");
...
        adminBot.forEach(e => {
          api.sendMessage`💀➣ 𝐈𝐃: ${event.threadID}\n🎃➣ Đã 𝐘ê𝐮 𝐂ầ𝐮 𝐏𝐡ê 𝐃𝐮𝐲ệ𝐭`, e);
        })
        return api.sendMessage(`💀➣ Đã 𝐊ê𝐮 𝐁ọ𝐧 𝐀𝐝𝐦𝐢𝐧 𝐇ã𝐦 𝐊𝐢𝐚 𝐃𝐮𝐲ệ𝐭\n🎃➣ 𝐗𝐢𝐧 𝐂𝐡ờ 𝐌ộ𝐭 𝐕à𝐢 𝐏𝐡ú𝐭`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`💀➣ 𝐁𝐨𝐱 𝐌à𝐲 Đã Đượ𝐜 𝐃𝐮𝐲ệ𝐭 Đâ𝐮 𝐌à 𝐃ù𝐧𝐠 𝐁𝐨𝐭?\n🎃➣ Để 𝐘ê𝐮 𝐂ầ𝐮 Đượ𝐜 𝐃𝐮𝐲ệ𝐭,𝐃ù𝐧𝐠:\n${prefix}𝐫𝐞𝐪𝐮𝐞𝐬𝐭`, event.threadID)
    };
